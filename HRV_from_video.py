from libs.processors_noopenmdao import findFaceGetPulse
from libs.interface import plotXY, imshow, waitKey, destroyWindow, resize
import cv2
...
class getPulseApp(object):
    ...
    def key_handler(self):
        ...
        if chr(self.pressed) == 's':
            self.toggle_search()
        if chr(self.pressed) == 'd':
            self.toggle_display_plot()
        if chr(self.pressed) == 'f':
            self.make_bpm_plot()
